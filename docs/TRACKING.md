...
- **NEW-5 fixed (P1, worst feel)**: the Execution zone (debt >= 50) was invisible; the loan guard can leave you at 49, the exempt interest tick pushes you past, and any later debt-add kills instantly. HUD now signals three states: white (safe) / amber (>= BREAK 30, collector coming) / red + explicit `hud.execution_warning` (>= EXECUTION 50, any new debt kills).
- Suite 181/181 after both fixes; i18n EN/ES parity for the new key.
**Deferred to user decisions (feel findings, not bugs):** price-hall effect of flat 15 vs the x1.5^n node economy; upgrade cap-2 forces early buying (no real timing choice); one-copy upgrade never becomes a power engine vs floor(debt/6) leverage; 55-77 dead gold in wins. Playtest summary: 4 runs, 2 wins (50%, in-sim band), 3.25/1.88/1.50/2.50 turns per combat.
...
| 2026-08-27 | **`card-upgrades` FIXED build, run 1/4 — baseline** — `develop` @ `48a7d6c`, physical `w8ono7aimjzlgyus`. Evidence: `temp/playtest/2026-08-27-fixed/partida-1-reporte.md` | **Victory** 8/8. Turns 3-2-4-2-3-3-4-5 (avg **3.25**, in the 2-4 band). 2 upgrades (Golpe 6→**9**, Defensa 5→**7**), 15 gold flat each. All four fix checkpoints pass: button drawn/tappable, **one** copy badged out of 5 Golpe, effective values correct in the combat log, offer order stable. Debt peaked at 14; **56 gold unspent**. New: NEW-1 (node opened trapped in the previous node's sub-screen), NEW-2 (ghost sub-screen offers inert, no feedback), NEW-3 (HP numbers unreadable). | None — playtest only, no code touched. |
| 2026-08-27 | **run 2/4 — "pay the debt off before the boss"** — same build/device. `partida-2-reporte.md` | **Defeat** at fight 8 (final Cobrador). Turns 2-2-2-1-1-2-3-2 (avg **1.88**, *below* the band). Cleared debt to 0 at node 7 → lost the whole `floor(debt/6)` leverage bonus on every attack and lost the damage race. **PAGAR DEUDA is a double trap** (escalating fee *and* removes damage) and nothing communicates the second half. Confirmed **Golpe Brutal cost 2→1, damage intact** (`baseCost < 2` gate). NEW-1 twice more. | None. |
| 2026-08-27 | **run 3/4 — debt-max, leverage exploit** — same build/device. `partida-3-reporte.md` | **Defeat** at fight 8 by **Execution**, not by HP. Turns 2-2-2-1-1-2-1-1 (avg **1.50**). Leverage measured in play = `floor(debt/6)` (debt 57 → **+9**), so fights collapsed to 1 turn past debt ~50. **NEW-5 (P1, worst finding):** PRÉSTAMO's guard is `debt + loan <= 50` *pre-interest* → lands at 49; turn-1 interest → 57; the interest tick is exempt from Execution ("Decision B"), so you survive several fights at 57-66 and conclude the threshold is inert — then any card adding ≥1 debt is an instant, unsignalled loss. Also NEW-6 (P3): `DebtConfig.kt:40-41` / `CardResolver.kt:116` still document `floor(debt/5)`. | None. |
| 2026-08-27 | **run 4/4 — control: upgrades only, debt 0** — same build/device. `partida-4-reporte.md` | **Victory** 8/8, the healthiest pacing of the four: turns 2-2-2-2-2-2-4-4 (avg **2.50**). Both upgrades bought at nodes 2-3. Cleanest one-copy proof of the session (one MEJORADA + **two** normal Golpe in one hand). **77 gold unspent**; from node 4 on, TOMAR CARTA was the only live button for five nodes straight. **NEW-7:** free-card rewards are debt-biased, so a zero-debt run gets dead cards — reached the boss with **two** "Burbuja de Activos" (damage = half debt = 0) in the opening hand. NEW-1 at its worst: node 2 inherited the ELIMINAR mode from **run 3** (mode survives `restartRun()`). | None. |
| 2026-08-27 | **`card-upgrades` FIXED verdict — FEATURE VALIDATED** (see `temp/playtest/2026-08-27-fixed/RESUMEN.md`) | All 3 prior P0s and the multi-copy balance bug are **gone**; the 4 checkpoints pass independently in all 4 runs. 2 wins / 4 runs (**50%**, matches the 52-55% sim band) but **neither defeat was a combat-difficulty loss** — both were uncommunicated economy rules (paying debt deletes your damage; crossing 50 kills instantly). New bugs: **P1** NEW-1 (node sub-mode never reset — observed **10×**, survives `restartRun()`; only `LOAN` self-resets, `CombatInputHandler.kt:116`), **P1** NEW-2 (ghost sub-screen accepts taps, does nothing, no feedback), **P1** NEW-5 (no signal anywhere above the Execution line); **P2** NEW-3 (player HP number covered by its own bar, enemy has none), NEW-4 (upgraded cards keep the static description text — cost updates, damage/block don't), NEW-7 (debt-biased rewards poison low-debt runs); **P3** NEW-6 (leverage divisor doc drift). Perf/stability clean: **119 fps sustained on a 120 Hz panel, 0 dropped frames, 0 crashes, 0 ANRs** over 17.4 min (`metrics.txt`, `crashcheck.txt`). | None — playtest only. Recommendation: fix NEW-1 **before** any further upgrade balance work (it contaminates every future measurement), then NEW-5. |
...
| Turns per combat | **2-4 (re-metriced 2026-08-27 — see note)** | **1.9 (sim, greedy sweep, RunSimulationHarnessTest)** · **2.28 human avg** (4 device runs 2026-08-27: 3.25 / 1.88 / 1.50 / 2.50). In band on average, but the band hides two opposite failures: debt-heavy runs collapse to **1 turn**, and only the zero-debt run stayed inside the band consistently — pacing tracks debt, not enemy HP | 🟢 re-metriced: the 8-10 target was a pre-pivot, StS-like assumption; the C2 debt-leverage design (compound interest, flat /6 leverage) resolves combats in ~2 turns by construction. Reaching 8-10 needs a full economy redesign (+25% enemy HP only reaches 2.1 and collapses win rate to 13%), so 2-4 is the designed band — reopening the economy is a post-launch design topic, not a P2 calibration |
...
| 60 FPS stability | 99% | **119.0 fps avg on a 120 Hz panel**, 0 frames >16.7 ms, 99.81% of seconds ≥110 fps (device `w8ono7aimjzlgyus`, 1042 one-second `BufferQueueProducer` samples over 17.4 min of play, 2026-08-27) | ✅ |
| Crash-free sessions | 100% | **100%** — 0 FATAL, 0 ANR, 0 tombstones, 0 process restarts across 4 full runs / 32 fights (506,216 logcat lines reviewed, 2026-08-27) | ✅ |
