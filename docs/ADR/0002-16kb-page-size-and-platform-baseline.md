...
time this rollback point was authored. `develop` has since advanced under a concurrent change
stream (C5, then C7) and the baseline has moved twice since; do not pin this figure to a SHA —
re-measure via `:app:testDebugUnitTest --rerun-tasks` and see `docs/HARNESS.md` for the current
count.
...
**Post-integration note (2026-08-27, sdd-verify P1, updated after C7 landed):** the 124/0/0
figures above are the ladder as actually run, against the tree P1 was originally authored on. Two
concurrent changes have landed on `develop` since — C5 (`RunSequenceTest`) and C7 (`NodeConfigTest`,
`ArchetypeTest`) — each moving the baseline again. This does not change any rung's PASS verdict —
each rung's test *surface* was unaffected by the platform bump either way — it only means 124/10
(and the 131/11 figure this note previously carried) are no longer the live number. Do not pin a
fixed test count to a specific SHA in this document; re-measure via `:app:testDebugUnitTest
--rerun-tasks` and see `docs/HARNESS.md`/`docs/TRACKING.md` for whatever the current baseline is.
