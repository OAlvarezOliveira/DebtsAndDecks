...
class DebtConfigTest {
    ...
    fun `applyInterest grows debt by the ceiling of the interest rate`() {
        // 100 * 10% = 10 exactly
        assertEquals(110, DebtConfig.applyInterest(100))
        // 3 * 10% = 0.3 -> ceil = 1
    ...
    fun `garnishAmount ramps up with debt below the break threshold`() {
        // debt=25 is half of BREAK_THRESHOLD(50) -> rate = 0.5 * MAX_GARNISH_RATE(0.6) = 0.3
        assertEquals(30, DebtConfig.garnishAmount(rawGold = 100, debt = 25))
    }
